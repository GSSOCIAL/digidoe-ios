...
extension ContactsView{
    ...
    func getContacts(page: Int = 1) async throws -> ContactsService.GetCustomersResponse?{
        ...
        if (self.selectedCustomerId.isEmpty){
            return nil;
        }
    ...
    func makePayment(_ contactId: String){
        Task{
            do{
                //Select individual account
                let contact = self.Store.contacts.list.first(where: {$0.contactId == contactId})
                if (contact != nil){
                    self.Store.payment.reference = ""
                    self.Store.payment.totalAmount = ""
                    self.Store.payment.totalPrice = ""
                    
                    let request = CoreCustomer.fetchRequest()
                    request.predicate = NSPredicate(format: "id == %@", self.selectedCustomerId)
    ...
    var customersList: Array<Option> {
        return self.customers.map({ customer in
            return Option(
                id: customer.id ?? "",
                label: customer.name ?? ""
            )
    ...
    func deleteContact(_ contactId: String) async throws{
        ...
        guard self.selectedCustomerId.isEmpty == false else{
            throw ApplicationError(title: "No customer", message: "Customer doesnt passed")
        }
        
        self.contactDeleteOTPResult = nil
        let initiate = try await services.contacts.initiateDeleteCustomerContacts(self.selectedCustomerId, contactId: contactId)
        ...
        Task{
            ...
            switch(self.contactDeleteOTPResult){
                case .confirmed(let operationId, let sessionId, let type):
                    do{
                        let isSuccess = try await services.contacts.finalizeContactOperation(self.selectedCustomerId, operationId: operationId, sessionId: sessionId, confirmationType: type)
                        if (isSuccess){
                            self.loading = false
    ...
    func copyContact(_ destination: CoreCustomer, contactId: String?) async throws{
        ...
        if (contact.currency.lowercased() == "usd"){
            if (contact.details.accountNumber?.isEmpty == false && contact.details.swiftCode?.isEmpty == false && contact.details.iban?.isEmpty == true){
                contact.details.iban = contact.details.accountNumber
                contact.details.accountNumber = ""
            }
        }
...
extension ContactsView{
    var popupActionsPopup: some View{
        VStack(spacing:0){
...
struct ContactsView: View, RouterPage {
    ...
    @State private var accountSelectPopup: Bool = false
    ...
    @State private var verifyCopyContact: Bool = false
    @State private var verifyCopyContactOperationId: String = ""
            ...
            if (self.selectedContact != nil && self.selectedContactId != nil){
                if (isContactDetailsMissing(self.selectedContact!) == false){
                    Button{
                        ...
                        self.makePayment(self.selectedContactId!)
                    } label:{
                        HStack{
                            Text("Make a payment")
                            Spacer()
                ...
                ZStack{
                    InfiniteScrollView(
                        ...
                        onScrollEnd: {
                            ...
                            Task{
                                do{
                                    try await self.getContacts(page: self.page + 1)
                                }catch(let error){
                                    self.loading = false
                                    self.Error.handle(error)
                                }
                        ...
                        VStack(spacing: 0){
                            VStack(spacing: 0){
                                ...
                                VStack{
                                    ...
                                    CustomField(
                                        value: self.$selectedCustomerId,
                                        placeholder: "Customer",
                                        type: .select,
                                        options: self.customersList,
                                        overrideSelect: false
                                    )
                                    .disabled(self.loading)
                                    .padding(.horizontal, 16)
                                    .onChange(of: self.selectedCustomerId){ _ in
                                        Task{
                                            do{
                                                self.page = 1
                            ...
                            .onAppear{
                                Task{
                                    do{
                                        let individual = CoreCustomer.fetchRequest()
                                        individual.predicate = NSPredicate(format: "lowercase:(type) == 'individual'")
                                        let individuals = try self.viewContext.fetch(individual)
                                        
                                        if (individuals.isEmpty){
                                            self.selectedCustomerId = self.customers.first?.id ?? ""
                                        }else{
                                            self.selectedCustomerId = individuals.first?.id ?? ""
                                        }
