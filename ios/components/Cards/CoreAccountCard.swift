//
//  CoreAccountCard.swift
//  DigiDoe Business Banking
//
//  Created by Настя Оксенюк on 05.12.2024.
//
...
struct CoreAccountCard: View{
    ...
    var availableBalance: Array<Substring> {
        return String(self.account.availableBalance?.value ?? 0).formatAsPrice(self.account.availableBalance?.currencyCode?.uppercased() ?? "").split(separator: ".")
    }
    
    var availableCurrency: String{
        return self.account.availableBalance?.currencyCode?.uppercased() ?? ""
    }
    
    var currentBalance: Array<Substring> {
        return String(self.account.currentBalance?.value ?? 0).formatAsPrice(self.account.currentBalance?.currencyCode?.uppercased() ?? "").split(separator: ".")
    }
        ...
        case .initial:
            VStack(spacing:0){
                ...
                VStack{
                    ...
                    HStack{
                        Text("Available balance: \(String(self.account.availableBalance?.value ?? 0).formatAsPrice(self.account.availableBalance?.currencyCode?.uppercased() ?? ""))")
                            .font(.caption)
                            .foregroundColor(Color.get(.Gray, scheme: .light))
                        Spacer()
                    }
